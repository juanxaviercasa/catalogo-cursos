...
| Configuración de ejemplo | `workers/video-processor.env.example` | Lista variables necesarias, sin secretos reales. |
...
| `VIDEO_PROCESSOR_URL` | Sí | Sí | URL HTTPS pública del servicio de vídeo. |
| `VIDEO_PROCESSOR_SHARED_SECRET` | Sí | Sí | Autentica cada solicitud de procesamiento. |
...
Publica el puerto mediante una URL HTTPS accesible por la plataforma web. Esa URL se registra como `VIDEO_PROCESSOR_URL`. Cuando importes un ZIP, la plataforma enviará los MKV en cola a tu equipo. Si apagas el equipo, los trabajos quedan visibles en cola y se procesan en cuanto el servicio vuelva a estar disponible.
...
3. Ejecuta las migraciones de `drizzle/` en orden y verifica las tablas `zip_imports`, `extracted_videos` y `media_tracks`.
...
6. Elige una sola ruta de trabajador, configura las tres variables `VIDEO_PROCESSOR_*` de la aplicación web y arranca el servicio correspondiente.
