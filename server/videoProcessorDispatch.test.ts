...
describe("disparo del procesador de vídeo", () => {
  it("no intenta una llamada externa cuando el trabajador no está configurado", async () => {
    ...
    expect(buildProcessorDispatchRequest({ id: 42, sourcePath: "curso/leccion.mkv" })).toBeNull();
    vi.unstubAllEnvs();
  });

  it("construye una solicitud autenticada para un trabajador configurado", async () => {
    vi.stubEnv("VIDEO_PROCESSOR_URL", "https://worker.example/");
    vi.stubEnv("VIDEO_PROCESSOR_SHARED_SECRET", "secreto-prueba");
    vi.stubEnv("VIDEO_PROCESSOR_MODE", "persistent-worker");
    const { buildProcessorDispatchRequest } = await import("./videoProcessorDispatch");
    const request = buildProcessorDispatchRequest({ id: 42, sourcePath: "curso/leccion.mkv" });
