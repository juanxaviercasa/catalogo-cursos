import { ArrowUpDown, FileText, Filter, Loader2, Save, Search } from "lucide-react";
...
type AdminDocument = PdfTranslationSummary & { courseTitle: string; moduleName: string };
...
export function PdfTranslationAdminPanel({ documents, onSetPriority, isSaving }: { documents: AdminDocument[]; onSetPriority: (id: number, priority: number) => void; isSaving?: boolean }) {
  ...
  const rows = useMemo(() => filterByPdfPriority(documents, priorityFilter).filter((document) => {
  ...
  }).sort((left, right) => left.priority - right.priority || left.moduleName.localeCompare(right.moduleName, "es")), [courseId, documents, priorityFilter, search, status]);
