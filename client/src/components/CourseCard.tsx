import type { CourseMeta } from "@shared/courseMeta";
  ...
  return (
    <article className="course-card">
      ...
      <div className="course-card-body">
        <p className="course-overline">{course.children.length} módulos · {videoCount ? `${videoCount} vídeos` : "biblioteca Drive"}</p>
        <h3>{meta.title}</h3>
        <p>{meta.description}</p>
      </div>
      <div className="course-card-footer">
        <span><FolderOpen size={14} /> Drive</span>
