  ...
  return (
    <div className="learning-app">
      <aside className="learning-sidebar">
        ...
        <div className="side-section"><span className="side-label">EXPLORAR</span><button className={`side-nav-item ${routeFilter === "all" && statusFilter === "all" ? "side-nav-item--active" : ""}`} onClick={() => { setRouteFilter("all"); setStatusFilter("all"); }}><LayoutGrid size={17} />Todos los cursos <b>19</b></button></div>
      ...
      <main className="learning-main">
        <header className="app-topbar"><div className="breadcrumbs"><span>Biblioteca</span><span>/</span><b>{routeFilter === "all" ? "Todas las rutas" : learningRoutes.find((route) => route.id === routeFilter)?.label}</b></div><a href="https://drive.google.com/drive/folders/1Rda2HOslYHwJUGK4vMR3MA8wHP19tNfX?usp=sharing" target="_blank" rel="noreferrer">Abrir Drive original <ArrowUpRight size={15} /></a></header>
        <section className="catalog-hero"><div><p className="eyebrow"><span /> BIBLIOTECA PERSONAL · DRIVE</p><h1>Aprende con<br /><em>dirección.</em></h1><p>Diecinueve cursos organizados en rutas que convierten una biblioteca extensa en una secuencia concreta: decide qué estudiar, abre el contenido y registra lo aprendido.</p></div><div className="hero-focus-card"><span>ENFOQUE DE HOY</span><h2>{inProgressCourses[0]?.meta.title ?? "Elige tu primera ruta"}</h2><p>{inProgressCourses[0] ? `${courseProgress(inProgressCourses[0].course)}% completado` : "Empieza por una ruta pedagógica para crear impulso."}</p><button onClick={() => inProgressCourses[0] ? setLocation(`/curso/${inProgressCourses[0].course.id}`) : setRouteFilter("business")}>{inProgressCourses[0] ? "Continuar curso" : "Explorar rutas"}<ArrowUpRight size={16} /></button></div></section>
